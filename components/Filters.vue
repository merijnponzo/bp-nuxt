...
<template>
  <div class="col col-12">
    <nav class="nav__filters">
      <a v-if="!mobile" @click="mobile = true" class="nav__filters--mobile"
        >&#8594; {{ $t("filter") }}</a
      >
      <ul
        class="nav__filters--panel"
        :class="{ active: mobile, 'filter-active': filterActive }"
      >
        <li v-if="mobile" class="nav__filters-mobile">
          <a class="nav__filters--close" @click="mobile = false"
            ><i class="ico-i_close"></i
          ></a>
        </li>
        <li
          class="nav__filters--all link-parent"
          :class="{ active: filterActive }"
        >
          <router-link
            to="/work"
            @click.native="
              (filterActive = null), (filtersGet = []), (mobile = true)
            "
          >
          ...
          <transition name="slide-fade" :key="'filter-sub-anim' + x">
            <template v-if="filterActive === null || filterActive === x">
              <li :key="'filter-sub' + x" v-if="filterActive === null">
                <a class="filter-link link-parent" @click="filterActive = x"
                  >{{ $t("per") }} {{ filterlist.name }}</a
                >
              </li>
              <template v-if="filterActive === x">
                <li class="nav__filters--childpanel">
                  <ol>
                    <li class="nav__filters--label fs__b xs">
                      {{ $t("per") }} {{ filterlist.name }}
                    </li>
                  </ol>
                  <ol class="nav__filters--childnav">
                    <li
                      v-for="(child, c) in filterlist.children"
                      :key="'child' + c"
